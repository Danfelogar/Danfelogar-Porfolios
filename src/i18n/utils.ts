...
export type UiType = keyof typeof uiText;

export function getLangFromUrl(url: URL) {
  const [, lang] = url.pathname.split("/");
  if (lang in uiText) return lang as UiType;
  return DEFAULT_LANG;
}

export function useTranslations(lang?: UiType) {
    return function t(
        key: keyof (typeof uiText)[typeof DEFAULT_LANG],
        ...args: any[]
    ) {
        let translation: string = uiText[lang ?? DEFAULT_LANG][key] || uiText[DEFAULT_LANG][key];
        if (args.length > 0) {
            for (let i = 0; i < args.length; i++) {
                translation = translation.replace(`{${i}}`, args[i] as string);
            }
        }
        return translation;
    };
